...
For collecting the assignments submitted to GitHub classroom, you have two options. You can either clone the repositories using GitHub Classroom assistant or use the `assignment.collect` function in `gradeR`, a wrapper for `ghclass:: local_repo_clone`. For instance, if you decide to use `gradeR` to clone the repos, you can use the following line of code.

```{r collect assingments, eval = FALSE}
assignment.collect(organization = "INFO123", GHpattern = "homework-1-F22", path = "HW1F22")
```
